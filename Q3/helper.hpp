...
public:
    ...
    void generate3AddressCode()
    {
        ...
        int count = 0;
        std::ofstream output_file("3AddressCode.txt"); // Create and open the output file.
        ...
        while (!st.empty())
        {
            ...
            if (mp.find(curr->curExp) != mp.end())
                continue;
            if (!curr->left && !curr->right)
            {
                mp[curr->curExp] = curr->val;
            }
            else if (curr->left && !curr->right)
            {
                mp[curr->curExp] = "t" + to_string(count++);
                output_file << mp[curr->curExp] << " := " << curr->val << mp[curr->left->curExp] << endl;
            }
            else
            {
                mp[curr->curExp] = "t" + to_string(count++);
                output_file << mp[curr->curExp] << " := " << mp[curr->left->curExp] << " " << curr->val << " " << mp[curr->right->curExp] << endl;
            }
        }
        output_file.close();
    }
